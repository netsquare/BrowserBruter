...
Usage Examples:
    1. Perform Bruteforce(ClusterBomb) on login page:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} username:username.txt,password:passwords.txt {GREEN}--target{RESET} http://localhost/login.php {GREEN}--button{RESET} Login {GREEN}--attack{RESET} 4 {GREEN}--remove-session{RESET}

    2. Perform PitchFork on login page:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} username:username.txt,password:passwords.txt {GREEN}--target{RESET} http://localhost/login.php {GREEN}--button{RESET} Login {GREEN}--attack{RESET} 3
    ...
    4. Fuzz on login page and make it verbose:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} username,password {GREEN}--fill{RESET} username,password {GREEN}--payloads{RESET} sqli.txt {GREEN}--target{RESET} http://localhost/login.php {GREEN}--button{RESET} Login {GREEN}--verbose{RESET} {GREEN}--attack{RESET} 1{RESET}
    ...
    7. Fuzz registration page with two cookies 1)'difficulty' and 2)'hint' and sent them forcefully on each request and remove session data and cookie after each request-response cycle [this is useful against Authentication pages when you don't want redirection in case of successful login]:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} name,age,address,phone {GREEN}--fill{RESET} name,age,address,phone {GREEN}--payloads{RESET} payloads.txt {GREEN}--target{RESET} http://localhost/register {GREEN}--button{RESET} register {GREEN}--cookie{RESET} difficulty:high,hint:no {GREEN}--force-cookie {GREEN}--remove-session {GREEN}--attack{RESET} 1{RESET}
    ...
    12. Fuzz <select> element - for example <select name="selectElement" required> <option value="">Select an option</option> <option value="option1">Option 1</option> </select>:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} selectElement {GREEN}--payloads{RESET} payloads.txt {GREEN}--target{RESET} http://localhost/selection {GREEN}--button{RESET} submit {GREEN}--attack{RESET} 1{RESET}
    ...
    41. Excluding urls from final report:
        {YELLOW}python3 BrowserBruter.py{GREEN} --target{RESET} http://localhost/webgoat/start.mvc#attack/1869022003/100 {GREEN}--elements{RESET} person {GREEN}--payloads{RESET} fuzz.txt {GREEN}--button{RESET} SUBMIT {GREEN}--attack{RESET} 1 {GREEN}--exclude-urls{RESET} "http://localhost/webgoat/service/solution.mvc","http://localhost/webgoat/service/lessonprogress.mvc","http://localhost/webgoat/service/lessonmenu.mvc","http://localhost/webgoat/service/solution.mvc"
        
        {RESET}OR -> Providing list of urls:
        {YELLOW}python3 BrowserBruter.py {GREEN}--target{RESET} http://localhost/webgoat/start.mvc#attack/1869022003/100 {GREEN}--elements{RESET} person {GREEN}--payloads{RESET} fuzz.txt {GREEN}--button{RESET} SUBMIT {GREEN}--attack{RESET} 1 {GREEN}--exclude-urls{RESET} urls.txt

    42. Including only specific urls in final report:
        {YELLOW}python3 BrowserBruter.py{GREEN} --target{RESET} http://localhost/webgoat/start.mvc#attack/1869022003/100 {GREEN}--elements{RESET} person {GREEN}--payloads{RESET} fuzz.txt {GREEN}--button{RESET} SUBMIT {GREEN}--attack{RESET} 1 {GREEN}--include-urls{RESET} "http://localhost/webgoat/attack?Screen=1869022003&menu=100"

        {RESET}OR -> Prvoding file containing URLs:
        {YELLOW}python3 BrowserBruter.py{GREEN} --target{RESET} http://localhost/webgoat/start.mvc#attack/1869022003/100 {GREEN}--elements{RESET} person {GREEN}--payloads{RESET} fuzz.txt {GREEN}--button{RESET} SUBMIT {GREEN}--attack{RESET} 1 {GREEN}--include-urls{RESET} urls.txt
    ...
    43. Force reload the page before fuzzing the form on each iteration, usefull when result of previous iteration causes the web elements to disappear or which leads to elements not found error, in such case this switch helps to keep browser bruter running:
        {YELLOW}python3 BrowserBruter.py{GREEN} --target{RESET} http://localhost/webgoat/start.mvc#attack/160587164/200 {GREEN}--elements{RESET} employee_id,password {GREEN}--button{RESET} submit {GREEN}--attack{RESET} 1 {GREEN}--cookie{RESET} ASP.NET_SessionId:fawx1oxcfm0fd4gvzmej5oco,JSESSIONID:1F0E3B20ECEDDF29235853F55C60FC17 {GREEN}--include-urls{RESET} "http://localhost/webgoat/attack?Screen=160587164&menu=200" {GREEN}--payloads{RESET} fuzz.txt {GREEN}--delay-before{RESET} 0.3 {GREEN}--delay-after{RESET} 0.3 {GREEN}--fill{RESET} employee_id,password {GREEN}--reload-page

    44. Print the stack trace for debugging purpose:
        {YELLOW}python3 BrowserBruter.py{GREEN} --target{RESET} http://localhost:13066/ {GREEN}--elements-payloads{RESET} username:username.txt,password:passwords.txt {GREEN}--button{RESET} btn-default {GREEN}--attack{RESET} 4 {GREEN}--debug{RESET}
    ...
    47. Become psycho and run 20 threads (unstable):
        {YELLOW}python3 BrowserBruter.py --target http://localhost/webgoat/login.mvc --attack 4 --elements-payloads exampleInputEmail1:username.txt,exampleInputPassword1:passwords.txt --button "btn.btn-large.btn-primary" --remove-session --threads 20 --exclude-urls urls.txt

        OR -> run in headless mode, this is most stable way to run in multple threads (still unstable):
        {YELLOW}python3 BrowserBruter.py --target http://localhost/webgoat/login.mvc --attack 4 --elements-payloads exampleInputEmail1:username.txt,exampleInputPassword1:passwords.txt --button "btn.btn-large.btn-primary" --remove-session --threads 20 --exclude-urls urls.txt --headless
    ...
    1. Perform BruteForce attack on login page:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} email:username.txt,password:passwords.txt {GREEN}--target{RESET} http://172.17.0.2/Lab3/login.php {GREEN}--attack{RESET} 4 {GREEN}--button{RESET} btn-secondary {GREEN}--remove-session{RESET}
    ...
    1. Perform brute force attack on login page to bypass authentication:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} username:username.txt,password:passwords.txt {GREEN}--button{RESET} btn-default {GREEN}--target{RESET} http://localhost:13066/ {GREEN}--attack{RESET} 4 {GREEN}--remove-session
{RESET}
    ...
    2. Perform Sniper attack on form page which is hidden, and requires pressing a button to be revealed, so use --interactive option to manually press the button:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} editBrandStatus,editBrandName,brandId {GREEN}--payloads{RESET} sqli.txt {GREEN}--button{RESET} editBrandBtn {GREEN}--target{RESET} http://localhost:13066/brand.php {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--attack{RESET} 1 {GREEN}--interactive {GREEN}--delay-after{RESET} 0.5 {GREEN}--fill{RESET} editBrandName,brandId,editBrandStatus

    3. Further automate the script by locating and clickng the buttons which enables the form:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} editBrandStatus,editBrandName,brandId {GREEN}--payloads{RESET} sqli.txt {GREEN}--button{RESET} editBrandBtn {GREEN}--target{RESET} http://localhost:13066/brand.php {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--attack{RESET} 1 {GREEN}--delay-before{RESET} 0.3 {GREEN}--fill{RESET} editBrandName {GREEN}--javascript{RESET} "document.querySelector('button.btn.btn-default.dropdown-toggle').click(); document.querySelector('a[data-target=\"#editBrandModel\"]').click();" {GREEN}--delay-after{RESET} 0.3{RESET}

    4. Provide above javascript code in file:    
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} editBrandStatus,editBrandName,brandId {GREEN}--payloads{RESET} sqli.txt {GREEN}--button{RESET} editBrandBtn {GREEN}--target{RESET} http://localhost:13066/brand.php {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--attack{RESET} 1 {GREEN}--delay-before{RESET} 1 {GREEN}--fill{RESET} editBrandName {GREEN}--javascript-file{RESET} click.js{RESET}
    ...
    5. Remove input validations form bootstrap.min.js file and press two buttons on web page using --javascript which will enable the web form:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements{RESET} editBrandStatus,editBrandName {GREEN}--payloads{RESET} fuzz.txt {GREEN}--button{RESET} editBrandBtn {GREEN}--target{RESET} http://localhost:13066/brand.php {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--attack{RESET} 1 {GREEN}--delay-before{RESET} 0.7 {GREEN}--fill{RESET} editBrandName {GREEN}--javascript{RESET} "document.querySelector('button.btn.btn-default.dropdown-toggle').click();document.querySelector('a[data-target=\"#editBrandModel\"]').click();" {GREEN}--delay-after{RESET} 0.5 {GREEN}--replace-files{RESET} "./res/samples/js-files-with-no-input-validation/bootstrap.min.js"++"http://localhost:13066/assests/bootstrap/js/bootstrap.min.js"{RESET}

    6. Brute force change password form with submit button's class name is 'btn btn-primary':
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} user_id:username.txt,password:passwords.txt,npassword:passwords.txt,cpassword:passwords.txt {GREEN}--target{RESET} http://localhost:13066/setting.php {GREEN}--attack{RESET} 4 {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--button{RESET} 'btn-primary'{RESET}

    7. Brute force change password form by selecting the specific form, because there is other form with same elements:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} user_id:username.txt,password:passwords.txt,npassword:passwords.txt,cpassword:passwords.txt {GREEN}--target{RESET} http://localhost:13066/setting.php {GREEN}--attack{RESET} 4 {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--button{RESET} 'btn-primary' {GREEN}--form{RESET} changePasswordForm{RESET}
 
    8. Brute force add user form but first press one using --buttons-to-press-before-fuzz to enable this form:
        {YELLOW}python3 BrowserBruter.py{GREEN} --elements-payloads{RESET} userName:fuzz.txt,upassword:fuzz.txt,uemail:fuzz.txt {GREEN}--button{RESET} createUserBtn {GREEN}--target{RESET} http://localhost:13066/user.php {GREEN}--attack{RESET} 4 {GREEN}--cookie{RESET} PHPSESSID:lqiqsdi8trjfeijdf8i2s2qru1 {GREEN}--buttons-to-press-before-fuzz{RESET} addUserModalBtn{RESET}
...
{RESET}
    ...
    {RED}USAGE EXAMPLES OF OTHER TOOLS:
    ...
    2. Grep the strings and show the counts:
        {YELLOW}python ReportExplore.py welcome,success,admin,root,error{RESET} 

    {BLUE}SPLIT REPORT{RESET}:
    
    1. Split the final report into small reports of 200 rows:
        {YELLOW}python SplitReport.py /path/to/report-file.csv 200{RESET}
''')